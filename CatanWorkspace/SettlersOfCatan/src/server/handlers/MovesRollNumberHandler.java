...
public class MovesRollNumberHandler extends SimpleHandler
{
	private static final Logger log = Logger.getLogger(MovesRollNumberHandler.class);
	
	private Server server = null;

	public MovesRollNumberHandler(Server server)
	{
		this.server = server;
	}
	...
	public void handle(HttpExchange exchange) throws IOException
	{
		...
		try
		{
			...
			RollNumberRequest request = getRequest(exchange, RollNumberRequest.class);
			
			ServerGame game = getGame(exchange, server);
			
			game.getMovesFacade().rollNumber(request);
