...
public class MovesRoadBuildingHandler extends SimpleHandler
{
	private static final Logger log = Logger.getLogger(MovesRoadBuildingHandler.class);
	
	private Server server = null;

	public MovesRoadBuildingHandler(Server server)
	{
		this.server = server;
	}
	...
	public void handle(HttpExchange exchange) throws IOException
	{
		...
		try
		{
			...
			RoadBuildingRequest request = getRequest(exchange, RoadBuildingRequest.class);
			
			ServerGame game = getGame(exchange, server);
			
			game.getMovesFacade().roadBuilding(request);
