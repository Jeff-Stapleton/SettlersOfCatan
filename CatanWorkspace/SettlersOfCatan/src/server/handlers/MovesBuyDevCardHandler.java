...
public class MovesBuyDevCardHandler extends SimpleHandler
{
	private static final Logger log = Logger.getLogger(MovesBuyDevCardHandler.class);
	
	private Server server = null;

	public MovesBuyDevCardHandler(Server server)
	{
		this.server = server;
	}
	...
	public void handle(HttpExchange exchange) throws IOException
	{
		try{
			...
			try
			{
				...
				BuyDevCardRequest request = getRequest(exchange, BuyDevCardRequest.class);
				
				ServerGame game = getGame(exchange, server);
				
				game.getMovesFacade().buyDevCard(request);
